...
label start:
    ...
    if persistent.playthrough == 0:
        $ chapter = 0
        call ch0_main                      # Intro/title sequence
        call poem                          # First poem writing sequence
        
        $ chapter = 1
        call ch1_main                      # Day 1 main gameplay
        call poemresponse_start            # Character reactions to player's poem
        call ch1_end                       # Day 1 ending
        call poem                          # Second poem writing sequence
        
        $ chapter = 2
        call ch2_main                      # Day 2 main gameplay
        call poemresponse_start            # Poem responses for day 2
        call ch2_end                       # Day 2 ending
        call poem                          # Third poem writing sequence
        
        $ chapter = 3
        call ch3_main                      # Day 3 main gameplay
        call poemresponse_start            # Poem responses for day 3
        call ch3_end                       # Day 3 ending
        
        $ chapter = 4
        call ch4_main                      # Day 4 (festival prep, Sayori confession)
        ...
        $ chapter = 5
        call ch5_main                      # Sayori death scene and restart
        
        call endgame                       # Show end screen
    ...
    # Playthrough 2 (Proper Act 2 start)
        ...
        label playthrough2:                # Common content for Act 2
            ...
            $ chapter = 1
            call ch21_main                 # Act 2 day 1 (glitched)
            call poemresponse_start        # Glitched poem responses
            call ch21_end                  # Act 2 day 1 ending
            call poem (False)              # Second poem with glitches
            ...
            $ chapter = 2
            call ch22_main                 # Act 2 day 2 (more glitches)
            call poemresponse_start        # More glitched responses
            call ch22_end                  # Act 2 day 2 ending
            call poem (False)              # Third poem with glitches
            
            $ chapter = 3
            call ch23_main                 # Act 2 day 3 (Yuri death)
            
            # Different response paths based on player's choices
            if y_appeal >= 3:              # If player favored Yuri
                call poemresponse_start2   # Special Yuri obsession path
            else:
                call poemresponse_start    # Standard response path
            ...
            call ch23_end                  # Weekend with Yuri scene
    ...
    # Playthrough 4 (Post-deletion - Act 4)
        ...
        $ chapter = 0
        call ch40_main                     # Final sequence with just Sayori
